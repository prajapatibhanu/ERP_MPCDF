...
public partial class mis_Masters_MilkItemRateMaster : System.Web.UI.Page
{
    DataSet ds;
    ...
    {
        Response.Redirect("MilkItemRateMaster.aspx", false);
    }
    ...
    protected void ddlitemtype_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                ds = null;
                ds = objdb.ByProcedure("USP_Mst_ItemSaleRate",
                             new string[] { "flag", "ItemCat_id", "ItemType_id", "Office_ID" },
                             new string[] { "2", ddlitemcategory.SelectedValue, ddlitemtype.SelectedValue, objdb.Office_ID() }, "dataset");
                if (ds != null)
                {
                    if (ds.Tables.Count > 0)
                    {
    ...
    private void BindSaleRate()
    {
        try
        {
            ...
            ds = null;
            ds = objdb.ByProcedure("USP_Mst_ItemSaleRate",
                                        new string[] { "flag", "Office_Id", "Item_id" },
                                        new string[] { "1", objdb.Office_ID(), objdb.DcsRawMilkItemId_ID() }, "dataset");
    ...
    protected void txtDistributorDCSPrice_TextChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                TextBox txtDistributorDCSPrice = (TextBox)row.FindControl("txtDistributorDCSPrice");
                TextBox txtDCSMargine = (TextBox)row.FindControl("txtDCSMargine");
                TextBox txtSecretaryPrice = (TextBox)row.FindControl("txtSecretaryPrice");

                TextBox txtSecretaryMargine = (TextBox)row.FindControl("txtSecretaryMargine");
                TextBox txtMRP = (TextBox)row.FindControl("txtMRP");

                if (txtDistributorDCSPrice.Text == "" || txtDCSMargine.Text == "")
                {
                    lblMsg.Text = objdb.Alert("fa-warning", "alert-warning", "Oops!", "Invalid Price / Margine");
                }
                else
                {
                    txtSecretaryPrice.Text = (Convert.ToDecimal(txtDistributorDCSPrice.Text) + Convert.ToDecimal(txtDCSMargine.Text)).ToString();
                }


                if (txtSecretaryPrice.Text == "" || txtSecretaryMargine.Text == "")
                {
                    lblMsg.Text = objdb.Alert("fa-warning", "alert-warning", "Oops!", "Invalid Price / Margine");
                }
                else
                {
                    txtMRP.Text = (Convert.ToDecimal(txtSecretaryPrice.Text) + Convert.ToDecimal(txtSecretaryMargine.Text)).ToString();
                }
    ...
    protected void txtDCSMargine_TextChanged(object sender, EventArgs e)
    {
    ...
    protected void txtSecretaryMargine_TextChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            foreach (GridViewRow row in gvItemDetails.Rows)
            {
                ...
                if (txtDistributorDCSPrice.Text == "" || txtDCSMargine.Text == "")
                {
                    lblMsg.Text = objdb.Alert("fa-warning", "alert-warning", "Oops!", "Invalid Price / Marginee");
                }
                else
                {
                    txtSecretaryPrice.Text = (Convert.ToDecimal(txtDistributorDCSPrice.Text) + Convert.ToDecimal(txtDCSMargine.Text)).ToString();
                ...
                if (ViewState["UPageTokan"].ToString() == Session["PageTokan"].ToString())
                {
                    ...
                    foreach (GridViewRow row in gvItemDetails.Rows)
                    {
                        ...
                        if (CheckBox1.Checked == true)
                        {
                            ...
                            if (txtSecretaryPrice.Text == "" || txtSecretaryPrice.Text == "0" || txtSecretaryPrice.Text == "0.00"
                                && txtMRP.Text == "" || txtMRP.Text == "0" || txtMRP.Text == "0.00")
                            {
                                CheckBlankVal = 1;
                            }
                        }
                    ...
                    if (gvItemDetails.Rows.Count > 0)
                    {
                        foreach (GridViewRow row in gvItemDetails.Rows)
                        {
                            ...
                            if (CheckBox1.Checked == true)
                            {
                                if (txtDistributorDCSPrice.Text == "" || txtDistributorDCSPrice.Text == "0"
                                    && txtMRP.Text == "" || txtMRP.Text == "0"
                                ...
                                else
                                {
                                    ...
                                    ds = objdb.ByProcedure("USP_Mst_ItemSaleRate",
                                               new string[] { "flag", "ItemCat_id", "ItemType_id", "Item_id", "Unit_id", "Office_ID", "DistributorDCSPrice", "DCSMargine", "SecretaryPrice", "SecretaryMargine", "MRP", "EffectiveDate", "CreatedBy" },
                                               new string[] { "0", lblItemCat_id.Text, lblItemType_id.Text, lblItem_id.Text, lblUnit_id.Text, objdb.Office_ID(), txtDistributorDCSPrice.Text, txtDCSMargine.Text, txtSecretaryPrice.Text, txtSecretaryMargine.Text, txtMRP.Text, date3.ToString(), objdb.createdBy() }, "dataset");
