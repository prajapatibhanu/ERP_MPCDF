...
public partial class mis_Finance_RptGraphicalPurchaseRegister : System.Web.UI.Page
{
    DataSet ds;
            ...
            if (Session["Emp_ID"] != null)
            {
                if (!IsPostBack)
                {
                    ...
                    if (Request.QueryString["OfficeID"] != null && Request.QueryString["FromDate"] != null && Request.QueryString["ToDate"] != null)
                    {
                        ...
                        if (Session["PurchaseHeading"].ToString() != null)
                        {
                            lblheadingFirst.Text = Session["PurchaseHeading"].ToString();
                        } 
    ...
    {
        ds = objdb.ByProcedure("SpFinRptGraphicalPurchaseRegister", new string[] { "flag", "Office_ID_Mlt", "FromDate", "ToDate" }, new string[] { "0", ViewState["OfficeID"].ToString(), Convert.ToDateTime(ViewState["FromDate"].ToString(), cult).ToString("yyyy/MM/dd"), Convert.ToDateTime(ViewState["ToDate"].ToString(), cult).ToString("yyyy/MM/dd") }, "dataset");
        ...
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {
            ...
            //sb.Append("title: {text: 'Graphical Representation Of Sales Register From (" + ViewState["FromDate"].ToString() + " - " + ViewState["ToDate"].ToString() + ")<br>(" + ViewState["OfficeName"].ToString() + ")'},");
