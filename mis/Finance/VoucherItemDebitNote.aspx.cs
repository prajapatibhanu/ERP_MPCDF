...
public partial class mis_Finance_VoucherItemDebitNote : System.Web.UI.Page
{
    DataSet ds;
    ...
    protected void FillVoucherNo()
    {
        try
        {
            ...
            string VoucherTx_Names_ForSno = "Item Debit Note Voucher";
            //string VoucherTx_Names_ForSno = "DebitNote Voucher";
            ...
            lblVoucherTx_No.Text = Office_Code + FinancialYear.ToString().Substring(2) + "IDN";
    ...
    protected void Save(string Type)
    {
        try
        {
            ...
            string VoucherTx_Type = "Item Debit Note Voucher";
            string VoucherTx_Name = "DebitNote Voucher";
                ...
                if (ds2.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int rowIndex = 0; rowIndex < gridRows; rowIndex++)
                    {
                        ...
                        if (ds != null && ds.Tables[0].Rows.Count > 0)
                        {
                            ...
                            {
                                string iAmount = "-" + lblAmount.Text;
                                string iQuantity = "-" + lblQuantity.Text;
                                objdb.ByProcedure("SpFinItemTx",
                                   ...
                                   , new string[] { "4", lblItemID.Text, iQuantity, "0", lblRate.Text, VoucherTx_ID, "Item Debit Note Voucher", VoucherTx_No, ViewState["Office_ID"].ToString(), lblWarehouse_id.Text, ViewState["Emp_ID"].ToString(), Convert.ToDateTime(txtVoucherTx_Date.Text, cult).ToString("yyyy/MM/dd"), iAmount }, "dataset");
                            }
                        }
                   ...
                   new string[] { "0", ddlPartyName.SelectedValue.ToString(),"Main Ledger",VoucherTx_ID,VoucherTx_Type,"-" + VoucherTx_Amount ,Month.ToString(),Year.ToString()
                        ...
                        for (int k = 0; k < gridChequeRows; k++)
                        {
                            ...
                            objdb.ByProcedure("SpFinChequeTx",
                                                 ...
                                                 new string[] { "1", VoucherTx_ID, ddlPartyName.SelectedValue.ToString(), "Item Debit Note Voucher", ChequeTx_No.Text, ChequeTx_Date.Text, ChequeTx_Amount.Text, Month.ToString(), Year.ToString(), FinancialYear.ToString(), ViewState["Office_ID"].ToString(), "0", ViewState["Emp_ID"].ToString(), (k + 1).ToString() }, "dataset");
                        }
                    }
                    else if (Type == "CostCentre")
                    {
                      ...
                      new string[] { "3", VoucherTx_ID }, "dataset");
                        foreach (GridViewRow row in GridCostCentreDetail.Rows)
                        {
                          ...
                          new string[] { "1", VoucherTx_ID, ddlPartyName.SelectedValue, Convert.ToDateTime(txtVoucherTx_Date.Text, cult).ToString("yyyy/MM/dd"), ViewState["Office_ID"].ToString(), lblCategory_ID.Text, lblSubCategory_ID.Text, lblAmount.Text, "Item Debit Note Voucher", "0", ViewState["Emp_ID"].ToString() }, "dataset");
                        }
                        objdb.ByProcedure("SpFinLedgerTx",
                              new string[] { "flag", "Ledger_ID","LedgerTx_Type", "VoucherTx_ID", "VoucherTx_Type", "LedgerTx_Amount", "LedgerTx_Month", "LedgerTx_Year"
                            , "LedgerTx_FY", "Office_ID","LedgerTx_IsActive", "LedgerTx_InsertedBy","LedgerTx_OrderBy","LedgerTx_MaintainType" },
        ...
        new string[] { "7",ViewState["VoucherTx_ID"].ToString(), Convert.ToDateTime(txtVoucherTx_Date.Text, cult).ToString("yyyy/MM/dd"), VoucherTx_Name, VoucherTx_Type,VoucherTx_No
                ...
                for (int rowIndex = 0; rowIndex < gridRows; rowIndex++)
                {
                    ...
                    if (ds != null && ds.Tables[0].Rows.Count > 0)
                    {
                        if (ds.Tables[0].Rows[0]["InventoryAffected"].ToString() == "Yes")
                        {
                            ...
                            objdb.ByProcedure("SpFinItemTx",
                                new string[] { "flag", "Item_id", "Cr", "Dr", "Rate", "TransactionID", "TransactionFrom", "InvoiceNo", "Office_Id", "Warehouse_id", "CreatedBy", "TranDt", "Amount" }
                               , new string[] { "4", lblItemID.Text, iQuantity, "0", lblRate.Text, ViewState["VoucherTx_ID"].ToString(), "Item Debit Note Voucher", VoucherTx_No, ViewState["Office_ID"].ToString(), lblWarehouse_id.Text, ViewState["Emp_ID"].ToString(), Convert.ToDateTime(txtVoucherTx_Date.Text, cult).ToString("yyyy/MM/dd"), iAmount }, "dataset");
                        }
                    }
                    string Amount = lblAmount.Text;
                    // Amount = "-" + Amount;
                ...
                objdb.ByProcedure("SpFinCostCentretx", new string[] { "flag", "VoucherTx_ID" }, new string[] { "3", VoucherTx_ID }, "dataset");
                if (Type == "BillByBill")
                {
               ...
               new string[] { "0", ddlPartyName.SelectedValue.ToString(),"Main Ledger",ViewState["VoucherTx_ID"].ToString(),VoucherTx_Type,"-" + VoucherTx_Amount ,Month.ToString(),Year.ToString()
                    ...
                    for (int k = 0; k < gridChequeRows; k++)
                    {
                        ...
                        objdb.ByProcedure("SpFinChequeTx",
                                             new string[] { "flag", "VoucherTx_ID", "Ledger_ID", "VoucherTx_Type", "ChequeTx_No", "ChequeTx_Date", "ChequeTx_Amount", "ChequeTx_Month", "ChequeTx_Year", "ChequeTx_FY", "Office_ID", "ChequeTx_IsActive", "ChequeTx_InsertedBy", "ChequeTx_OrderBy" },
                                             new string[] { "1", ViewState["VoucherTx_ID"].ToString(), ddlPartyName.SelectedValue.ToString(), "Item Debit Note Voucher", ChequeTx_No.Text, ChequeTx_Date.Text, ChequeTx_Amount.Text, Month.ToString(), Year.ToString(), FinancialYear.ToString(), ViewState["Office_ID"].ToString(), "1", ViewState["Emp_ID"].ToString(), (k + 1).ToString() }, "dataset");
    ...
    protected void btnNarration_Click(object sender, EventArgs e)
    {
        ds = objdb.ByProcedure("SpFinVoucherTx",
                 ...
                 new string[] { "14", "Item Debit Note Voucher", ViewState["Office_ID"].ToString() }, "dataset");
    ...
    protected void lnkPreviousVoucher_Click(object sender, EventArgs e)
    {
        ...
        {
            lblMsg.Text = "";
            ds = objdb.ByProcedure("SpFinVoucherTx", new string[] { "flag", "Office_ID", "VoucherTx_Type" }, new string[] { "33", ViewState["Office_ID"].ToString(), "Item Debit Note Voucher" }, "dataset");
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
        ...
        try
        {
            ...
            if (ds != null)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    var rx = new System.Text.RegularExpressions.Regex("IDN");
                    string str = ds.Tables[0].Rows[0]["VoucherTx_No"].ToString();
                    var array = rx.Split(str);
                    lblVoucherNo.Text = ds.Tables[0].Rows[0]["VoucherTx_No"].ToString();
                    txtVoucherTx_No.Text = array[1];
                    lblVoucherTx_No.Text = array[0] + "IDN";
                    txtVoucherTx_Date.Text = ds.Tables[0].Rows[0]["VoucherTx_Date"].ToString();
                    ViewState["VoucherTx_Date"] = ds.Tables[0].Rows[0]["VoucherTx_Date"].ToString();
                    ViewState["FY"] = ds.Tables[0].Rows[0]["VoucherTx_FY"].ToString();
                    lblGrandTotal.Text = ds.Tables[0].Rows[0]["VoucherTx_Amount"].ToString();
