...
public partial class mis_Finance_VoucherPurchaseGoodsGSTTaxFree : System.Web.UI.Page
{
    DataSet ds;
    ...
    protected void FillVoucherNo()
    {
        try
        {
            ...
            string VoucherTx_Names_ForSno = "Goods Purchase Tax Free";
    ...
    protected void Save(string Type)
    {
        try
        {
            ...
            string VoucherTx_Type = "Goods Purchase Tax Free";
            string VoucherTx_Name = "Goods Purchase Tax Free";
        ...
        new string[] { "0", Convert.ToDateTime(txtVoucherTx_Date.Text, cult).ToString("yyyy/MM/dd"), VoucherTx_Name, VoucherTx_Type,VoucherTx_No
                       ,VoucherTx_Ref,txtVoucherTx_Narration.Text,lblGrandTotal.Text,Month.ToString(),Year.ToString(), ViewState["Office_ID"].ToString(),FinancialYear.ToString()
                        ,VoucherTx_IsActive,ViewState["Emp_ID"].ToString(),Convert.ToDateTime(txtSupplierInvoiceDate.Text, cult).ToString("yyyy/MM/dd"),"No"}, "dataset");
                        ...
                        if (ds != null && ds.Tables[0].Rows.Count > 0)
                        {
                            if (ds.Tables[0].Rows[0]["InventoryAffected"].ToString() == "Yes")
                            {
                                objdb.ByProcedure("SpFinItemTx",
                                   ...
                                   , new string[] { "4", lblItemID.Text, lblQuantity.Text, "0", lblRate.Text, VoucherTx_ID, "Goods Purchase Tax Free", VoucherTx_No, ViewState["Office_ID"].ToString(), lblWarehouse_id.Text, ViewState["Emp_ID"].ToString(), Convert.ToDateTime(txtVoucherTx_Date.Text, cult).ToString("yyyy/MM/dd") }, "dataset");
                            }
                        }
                ...
                for (int rowIndex = 0; rowIndex < gridRows; rowIndex++)
                {
                    ...
                    if (ds != null && ds.Tables[0].Rows.Count > 0)
                    {
                        if (ds.Tables[0].Rows[0]["InventoryAffected"].ToString() == "Yes")
                        {
                            objdb.ByProcedure("SpFinItemTx",
                                new string[] { "flag", "Item_id", "Cr", "Dr", "Rate", "TransactionID", "TransactionFrom", "InvoiceNo", "Office_Id", "Warehouse_id", "CreatedBy", "TranDt" }
                               , new string[] { "4", lblItemID.Text, lblQuantity.Text, "0", lblRate.Text, ViewState["VoucherTx_ID"].ToString(), "Goods Purchase Tax Free", VoucherTx_No, ViewState["Office_ID"].ToString(), lblWarehouse_id.Text, ViewState["Emp_ID"].ToString(), Convert.ToDateTime(txtVoucherTx_Date.Text, cult).ToString("yyyy/MM/dd") }, "dataset");
                        }
                    }
                    string Amount = lblAmount.Text;
                    Amount = "-" + Amount;
    ...
    {
        //string ValidStatus = "Yes";//ValidDate();
		string ValidStatus =ValidDate();
        if (ValidStatus == "No")
        {
    ...
    protected void btnNarration_Click(object sender, EventArgs e)
    {
        ds = objdb.ByProcedure("SpFinVoucherTx",
                 ...
                 new string[] { "14", "Goods Purchase Tax Free", ViewState["Office_ID"].ToString() }, "dataset");
    ...
    protected void lnkPreviousVoucher_Click(object sender, EventArgs e)
    {
        ...
        {
            lblMsg.Text = "";
            ds = objdb.ByProcedure("SpFinVoucherTx", new string[] { "flag", "Office_ID", "VoucherTx_Type" }, new string[] { "33", ViewState["Office_ID"].ToString(), "Goods Purchase Tax Free" }, "dataset");
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {                                          
    ...
    protected string ValidDate()
    {
        ...
        if (txtVoucherTx_Date.Text != "")
        {
            ...
            DataSet dsValidDate = objdb.ByProcedure("SpFinEditRight", new string[] { "flag", "Office_ID", "VoucherDate", "FinancialYear","VoucherTx_Type" }, new string[] { "3", ViewState["Office_ID"].ToString(), Convert.ToDateTime(txtVoucherTx_Date.Text, cult).ToString("yyyy/MM/dd"), FinancialYear,"Goods Purchase Tax Free" }, "dataset");
