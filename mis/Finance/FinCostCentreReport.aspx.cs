...
public partial class mis_Finance_FinCostCentreReport : System.Web.UI.Page
{
    DataSet ds;
    ...
    protected void FillGrid()
    {
        try
        {
            ...
            string headingFirst = "<p class='text-center' style='font-weight:600'>" + ViewState["Office_FinAddress"].ToString() + " <br />  Cost Centre Report<br />" + Convert.ToDateTime(txtFromDate.Text, cult).ToString("dd-MM-yyyy") + "  To " + Convert.ToDateTime(txtToDate.Text, cult).ToString("dd-MM-yyyy") + "</p>";
            lblheadingFirst.Text = headingFirst;

            ds = objdb.ByProcedure("Sp_Fin_CostCentreReport", new string[] { "flag", "OfficeID", "FromDate", "ToDate" }, new string[] { "1", Office, Convert.ToDateTime(txtFromDate.Text, cult).ToString("yyyy/MM/dd"), Convert.ToDateTime(txtToDate.Text, cult).ToString("yyyy/MM/dd") }, "dataset");
            if (ds.Tables.Count != 0 && ds.Tables[0].Rows.Count != 0)
            {
    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "View")
        {
            ...
            FillCategoryWiseRecord(lblCategoryId.Text, lblCategoryName.Text);
        }
    }
        ...
        if (e.CommandName == "View")
        {
            ...
            if (debit != 0 || credit != 0)
            {
                ...
                ds = objdb.ByProcedure("Sp_Fin_CostCentreReport", new string[] { "flag", "OfficeID", "FromDate", "ToDate", "Category_ID", "SubCategoryId", "MonthID" },
                  new string[] { "4", Office, Convert.ToDateTime(txtFromDate.Text, cult).ToString("yyyy/MM/dd"), Convert.ToDateTime(txtToDate.Text, cult).ToString("yyyy/MM/dd"), ViewState["CategoryID"].ToString(), ViewState["SubCategoryId"].ToString(), lblMonthID.Text }, "dataset");
    ...
    protected void btnBack_Click(object sender, EventArgs e)
    {
        ...
        else if (ViewState["GridCount"].ToString() == "4")
        {
            GridView1.Visible = false;
            GridView2.Visible = false;
            GridView3.Visible = false;
