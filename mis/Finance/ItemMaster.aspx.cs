    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (msg.Trim() == "")
            {
                ...
                {
                    ds = objdb.ByProcedure("SpItemMaster",
                                new string[] { "flag", "GroupId", "CategoryId", "UnitId", "PurchaseLedger_id", "SalesLedger_id", "ItemName", "ItemName_Hindi", "ItemAliasCode", "CreatedBy", "HSNCode", "ItemBrand", "ItemSize", "ItemSpecification", "LengthClass", "PackagingSize", "Packaging_Mode", "ItemOrderNo", "Office_ID", "CreatedAt", "CreatedByIP"},
                                new string[] { "1", ItemGroup.SelectedValue, ddlItemCategory.SelectedValue, ddlUnit.SelectedValue, ddlpurchaseledger.SelectedValue.ToString(), ddlsalesledger.SelectedValue.ToString(), txtItemName.Text, txtItemNameHindi.Text, txtitemaliscode.Text, ViewState["Emp_ID"].ToString(), ddlHsnCode.SelectedItem.Text, txtItemBrand.Text, txtItemSize.Text, txtItemSpecification.Text, ddlDimensionClass.SelectedItem.Text, txtPackgngSz.Text, ddlPackgngMode.SelectedValue.ToString(), txtItemOrderNo.Text,objdb.Office_ID(),objdb.Office_ID(),objdb.GetLocalIPAddress() }, "dataset");
                    if (ds != null && ds.Tables[0].Rows.Count > 0)
                    {
