...
public partial class mis_MilkCollection_CCWiseSocietywithandwithoutincentiveamount : System.Web.UI.Page
{
    DataSet ds;
        ...
        try
        {
            ...
            if (dsFillGrid != null && dsFillGrid.Tables.Count > 0)
            {
                if (dsFillGrid.Tables[0].Rows.Count > 0)
                {
                    ...
                    spn.InnerHtml = Session["Office_Name"] + "<br/>" + "Society with & without Incentive Amount" + "<br/>  CC:" + ddlccbmcdetail.SelectedItem.Text + " ( " + txtFdt.Text + " - " + txtTdt.Text + " )";
                    GridView1.DataSource = dsFillGrid.Tables[0];
                    GridView1.DataBind();
                    string TotalQuantity = "0";
                    string TotalAmount = "0";
                    TotalQuantity = dsFillGrid.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("Quantity")).ToString();
                    TotalAmount = dsFillGrid.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("Amount")).ToString();
                    GridView1.FooterRow.Cells[2].Text = "<b>Total : </b>";
                    GridView1.FooterRow.Cells[3].Text = "<b>" + TotalQuantity.ToString() + "</b>";
                    GridView1.FooterRow.Cells[7].Text = "<b>" + TotalAmount.ToString() + "</b>";
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Response.Clear();
            Response.AddHeader("content-disposition", "attachment;filename=" + "Societywith&withoutIncentiveAmount" + DateTime.Now + ".xls");
            Response.Charset = "";
