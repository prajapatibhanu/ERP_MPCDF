...
public partial class mis_MilkCollection_UDSProducerDeathClaimMaster : System.Web.UI.Page
{
    DataSet ds;
    static DataSet ds5;
        ...
        try
        {
            if (Page.IsValid)
            {
                if (ViewState["UPageTokan"].ToString() == Session["PageTokan"].ToString())
                {
                    ...
                    dt = GetData();
                    ds = objdb.ByProcedure("Usp_UDSProducerDeathClaim", 
                                         new string[] {"flag",
                                                       "Office_ID",
                                                       "EntryDate",
                                         ...
                                         new string[] {"1",
                                                      ...
                                                      }, new string[] { "type_UDSProducerDeathClaim" }, new DataTable[] { dt }, "TableSave");
    ...
    protected void FillGrid()
    {
        try
        {
            ...
            GridView1.DataBind();
            ds = objdb.ByProcedure("Usp_UDSProducerDeathClaim", new string[] { "flag", "Office_ID" }, new string[] {"2",objdb.Office_ID() }, "dataset");
            if (ds != null && ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
    ...
    protected void FillCategoryGrid()
    {
        try
        {
            ...
            dt.Columns.Add("DeathClaimAmount", typeof(string));
    ...
    protected DataTable GetData()
    {
        ...
        foreach(GridViewRow rows in gvCategory.Rows)
        {
            Label lblCategory = (Label)rows.FindControl("lblCategory");
            TextBox txtDeathClaimAmount = (TextBox)rows.FindControl("txtDeathClaimAmount");

            dt.Rows.Add(lblCategory.Text, txtDeathClaimAmount.Text);
        }
        return dt;
    }
}