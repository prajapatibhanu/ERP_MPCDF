...
public partial class mis_MilkCollection_RMRDChallanEntryReportViaCanesCollection : System.Web.UI.Page
{
    DataSet ds;
    ...
    protected void FillGrid()
    {
        try
        {
            ...
            ds = objdb.ByProcedure("Usp_MilkCollectionChallanEntryViaCanes", new string[] { "flag", "FromDate", "ToDate", "CC_Id", "Office_Parant_ID", "OfficeType_ID" }, new string[] { "11", FromDate, ToDate, ddlCC.SelectedValue, objdb.Office_ID(),objdb.OfficeType_ID() }, "dataset");
            if (ds != null && ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    TotalBFAT = ds.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("BD_FAT"));
                    TotalBClr = ds.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("BD_CLR"));
                    TotalBSnf = ds.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("BD_SNF"));
                    TotalBFATInKg = ds.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("BFatInKg"));
                    TotalCFAT = ds.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("CD_FAT"));
                    TotalCClr = ds.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("CD_CLR"));
                    TotalCSnf = ds.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("CD_SNF"));
    ...
    protected void btnExcel_Click(object sender, EventArgs e)
    {
        try
        {
            string FileName = "CanesCollectionSheetReport";
            Response.Clear();
    ...
    private DataTable GenerateData()
    {
        ...
        ds = objdb.ByProcedure("Usp_MilkCollectionChallanEntryViaCanes", new string[] { "flag", "Office_Parant_ID", "FromDate", "ToDate", "CC_Id", "OfficeType_ID" }, new string[] { "12", objdb.Office_ID(), Convert.ToDateTime(txtFromDate.Text, cult).ToString("yyyy/MM/dd"), Convert.ToDateTime(txtToDate.Text, cult).ToString("yyyy/MM/dd"), ddlCC.SelectedValue ,objdb.OfficeType_ID()}, "dataset");
        if (ds != null && ds.Tables.Count > 0)
        {
