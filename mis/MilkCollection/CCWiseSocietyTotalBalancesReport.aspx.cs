...
public partial class mis_MilkCollection_CCWiseSocietyTotalBalancesReport : System.Web.UI.Page
{
    DataSet ds;
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ds = objdb.ByProcedure("Usp_MilkCollectionRoutWiseAdditionsDeductionsEntry", new string[] { "flag", "FromDate", "ToDate", "Office_ID", "Office_Parant_ID", "OfficeType_ID" }, new string[] { "20", Fdate, Tdate, ddlccbmcdetail.SelectedValue, objdb.Office_ID(),objdb.OfficeType_ID() }, "dataset");
            if(ds != null && ds.Tables.Count > 0)
            {
                if(ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    decimal TotalBalance = Math.Round(ds.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("BalanceAmount")),0);
                    decimal GrossEarning = Math.Round(ds.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("GrossEarning")), 0);
                    decimal Diff = Math.Round(ds.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("HighRiskNegativePayment")), 0);
                    ...
                    GridView1.FooterRow.Cells[3].Text = "<b>" + TotalBalance.ToString() + "</b>";
                    GridView1.FooterRow.Cells[2].Text = "<b>" + GrossEarning.ToString() + "</b>";
                    GridView1.FooterRow.Cells[4].Text = "<b>" + Diff.ToString() + "</b>";
                }
                else
                {
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            lblMsg.Text = "";
            string FileName = Session["Office_Name"].ToString() + "_" + "CCWiseSocietyNegativeBalancesReport_" + ddlccbmcdetail.SelectedItem.Text + "_";
            Response.Clear();
