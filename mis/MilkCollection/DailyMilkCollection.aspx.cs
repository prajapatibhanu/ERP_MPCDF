...
public partial class mis_MilkCollection_DailyMilkCollection : System.Web.UI.Page
{
    DataSet ds;  
        ...
        try
        {
            if (ViewState["UPageTokan"].ToString() == Session["PageTokan"].ToString())
            {
                ...
                if (btnSubmit.Text == "Submit")
                {
                    ds = null;
                    //ds = objdb.ByProcedure("sp_Mst_DailyMilkCollection",
                    //             new string[] { "flag", "Office_Id", "V_SocietyName", "Dt_Date", "V_Shift", "I_Producer_ID", "I_MilkSupplyQty", "V_MilkType", "V_Review", "CreatedBy", "CreatedBy_IP" },
                    //             new string[] { "1", ViewState["Office_ID"].ToString(), txtSociatyName.Text, Convert.ToDateTime(txtDate.Text, cult).ToString("yyyy/MM/dd"), ddlShift.SelectedValue.ToString(), ddlFarmer.SelectedValue.ToString(), txtI_MilkQuantity.Text, ddlMilkType.SelectedValue.ToString(), txtRemark.Text, ViewState["Emp_ID"].ToString(), objdb.GetLocalIPAddress() }, "dataset");
                    ds = objdb.ByProcedure("sp_Mst_DailyMilkCollection",
                    new string[] { "Flag"
                     ,"Office_Id"
                     ,"V_SocietyName"
    ...
    protected void gvMilkCollection_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName == "RecordDelete")
            {
                if (ViewState["UPageTokan"].ToString() == Session["PageTokan"].ToString())
                {
                    ViewState["I_CollectionID"] = e.CommandArgument;
                    lblMsg.Text = string.Empty;

                    objdb.ByProcedure("sp_Mst_DailyMilkCollection",
                            new string[] { "flag", "I_CollectionID", "DeletedBy", "DeletedBy_IP" },
                            new string[] { "8", ViewState["I_CollectionID"].ToString(), ViewState["Emp_ID"].ToString(), objdb.GetLocalIPAddress() }, "dataset");

                    lblMsg.Text = objdb.Alert("fa-check", "alert-success", "Thank you!", "Record Deleted Successfully");
                    //if (ds.Tables[0].Rows[0]["Msg"].ToString() == "Ok")
                    //{
                    //    lblMsg.Text = objdb.UpdateMessage(ds.Tables[0].Rows[0]["ErrorMsg"].ToString());
                    //}
                    //else
                    //{
                    //    lblMsg.Text = objdb.ErrorMsg(ds.Tables[0].Rows[0]["ErrorMsg"].ToString());
                    //}
            ...
            if (e.CommandName == "RecordUpdate")
            {
                ViewState["I_CollectionID"] = e.CommandArgument;
                lblMsg.Text = string.Empty;
                ds = objdb.ByProcedure("sp_Mst_DailyMilkCollection", new string[] { "flag", "I_CollectionID" }, new string[] { "7", ViewState["I_CollectionID"].ToString() }, "dataset");
                if (ds.Tables[0].Rows.Count > 0)
                {
                    txtSociatyName.Text = ds.Tables[0].Rows[0]["V_SocietyName"].ToString();
                    txtDate.Text = ds.Tables[0].Rows[0]["Dt_Date"].ToString();
                    ddlShift.ClearSelection();
                    ddlShift.SelectedValue = ds.Tables[0].Rows[0]["V_Shift"].ToString();
                    //txtCode.Text = ds.Tables[0].Rows[0]["V_Code"].ToString();
                    //txtAssembly.Text = ds.Tables[0].Rows[0]["V_Assembly"].ToString();
                    //txtTown.Text = ds.Tables[0].Rows[0]["V_Town"].ToString();
                    ddlFarmer.ClearSelection();
                    ddlFarmer.SelectedValue = ds.Tables[0].Rows[0]["I_Producer_ID"].ToString();
                    // txtID.Text = ddlFarmer.SelectedValue.ToString();
                    txtI_MilkQuantity.Text = ds.Tables[0].Rows[0]["I_MilkSupplyQty"].ToString();
                    txtRemark.Text = ds.Tables[0].Rows[0]["V_Review"].ToString();
                }
                btnSubmit.Text = "Update";
                foreach (GridViewRow gvRow in gvMilkCollection.Rows)
                {
                    if (gvMilkCollection.DataKeys[gvRow.DataItemIndex].Value.ToString() == e.CommandArgument.ToString())
                    {
                        gvMilkCollection.SelectedIndex = gvRow.DataItemIndex;
                        gvMilkCollection.SelectedRowStyle.BackColor = System.Drawing.Color.LightBlue;
                        break;
                    }
                }
    ...
    protected void FillGrid()
    {
        try
        {
            ...
            ds = objdb.ByProcedure("sp_Mst_DailyMilkCollection",
                              ...
                              new string[] { "19", strdt, ddlShift.SelectedValue.ToString(), objdb.Office_ID() }, "dataset");
            if (ds.Tables[0].Rows.Count > 0)
            {
    ...
    protected void gvMilkCollection_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            foreach (GridViewRow row1 in gvMilkCollection.Rows)
            {
                Label lblCLR = (Label)row1.FindControl("lblCLR");
                Label lblFat = (Label)row1.FindControl("lblFat");
                TextBox txtI_MilkSupplyQty = (TextBox)row1.FindControl("txtI_MilkSupplyQty");

                if (lblCLR.Text != "" && lblFat.Text != "")
                {
                    txtI_MilkSupplyQty.Enabled = false;
                    txtI_MilkSupplyQty.ToolTip = "Fat-" + lblFat.Text + " CLR-" + lblCLR.Text + " Inserted";
                }
    ...
    private decimal GetSNF()
    {
        ...
        try
        {
            if (txtNetCLR.Text != "") { CLR = Convert.ToDecimal(txtNetCLR.Text); }
