...
public partial class mis_Common_ProductVariantMaster : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
 {
    ...
    protected void GetProduct()
    {
        try
        {
            ...
            {
                ddlProductName.DataSource = objdb.ByProcedure("SptblPUProductMaster",
                  new string[] { "flag", "Cat_id" },
                  new string[] { "6", ddlProductCategory.SelectedValue }, "dataset");
                ddlProductName.DataTextField = "PNameOrAbbre";
    ...
    private void InsertOrUpdateProduct()
    {
        if (Page.IsValid)
        {
            try
            {
                ...
                {
                    DateTime date3 = DateTime.ParseExact(txtEffectiveDate.Text.Trim(), "dd/MM/yyyy", culture);
                    if (btnSubmit.Text == "Save")
                    {
                        lblMsg.Text = "";
                        ds = objdb.ByProcedure("SptblPUProductVariant",
                            new string[] { "flag", "Cat_id", "ProductId"
                                        , "PacketSize", "MRPRate", "EffectiveDate"
                    ...
                    else if (btnSubmit.Text == "Update")
                    {
                        lblMsg.Text = "";
                        ds = objdb.ByProcedure("SptblPUProductVariant",
                                 new string[] { "flag", "VariantId", "MRPRate", "EffectiveDate", "CreatedBy", "CreatedBy_IP", "PageName", "Remark" },
