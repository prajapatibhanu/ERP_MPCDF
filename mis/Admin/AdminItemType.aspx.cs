                ...
                if (ViewState["UPageTokan"].ToString() == Session["PageTokan"].ToString())
                {
                    if (btnSubmit.Text == "Save")
                    {
                        ...
                        ds = objdb.ByProcedure("SpItemType",
                            new string[] { "flag", "ItemCat_id", "ItemTypeName", "Abbreviation", "Description", "CreatedBy", "CreatedBy_IP" },
                            new string[] { "2",ddlItemCategory.SelectedValue,txtItemTypeName.Text.Trim(), txtAbbreviation.Text.Trim(), txtDescription.Text.Trim(), objdb.createdBy(), objdb.GetLocalIPAddress() + ":" + objdb.GetMACAddress() }, "TableSave");
                        ...
                        else
                        {
                            ...
                            {
                                lblMsg.Text = objdb.Alert("fa-warning", "alert-warning", "Warning!", txtItemTypeName.Text + " or " + txtAbbreviation.Text.Trim() + " " + error);
                            }
                            else
                            {
                    ...
                    if (btnSubmit.Text == "Update")
                    {
                        lblMsg.Text = "";
                        ds = objdb.ByProcedure("SpItemType",
                            new string[] { "flag", "ItemType_id", "ItemCat_id", "ItemTypeName", "Abbreviation", "Description", "CreatedBy", "CreatedBy_IP", "PageName", "Remark" },
                            new string[] { "3", ViewState["rowid"].ToString(), ddlItemCategory.SelectedValue
                                , txtItemTypeName.Text.Trim(),txtAbbreviation.Text.Trim(), txtDescription.Text.Trim()
                                , objdb.createdBy()
                                , objdb.GetLocalIPAddress() + ":" + objdb.GetMACAddress(), Path.GetFileName(Request.Url.AbsolutePath), "ItemType Master Record Updated" }, "TableSave");
                        if (ds.Tables[0].Rows[0]["Msg"].ToString() == "Ok")
                        {
                            string success = ds.Tables[0].Rows[0]["ErrorMsg"].ToString();
                            Clear();
