...
public partial class mis_Admin_ChangePassword : System.Web.UI.Page
{
    DataSet ds, ds3;
    ...
    public string SendOTP(string Email)
    {
       ...
       mail.To.Add(Email);
        ...
        mail.Body = "Dear" + " " + Session["Emp_Name"] + ",<br/><br/><br/>" + "Your Change Password OTP is   " + otp + "<br/><br/>" + "This is a system generated e-mail and please do not reply.";
        ...
        if (txtOldPassword.Text != "" && txtNewPassword.Text != "" && txtConfirmPassword.Text != "")
        {
            switch (apiprocedure.GetUserName().Substring(0, 1))
            {
                ...
                case "D": //for Distributor Login
					...
					case "S": //for Super Stockist Login
                            ds3 = apiprocedure.ByProcedure("USP_Mst_SuperStockistReg",
                                    new string[] { "flag", "UserName" },
                                    new string[] { "0", apiprocedure.GetUserName().ToString() }, "dataset");
                            break;
                case "B": //for Booth/Parlour Login
                ...
                case "M": //for Temp Employees Login of offices like HO, DS, DCS, CC, BMC, MDP 
                    ds3 = apiprocedure.ByProcedure("usp_Mst_McuUserMaster",
                            new string[] { "flag", "UserName" },
                            new string[] { "0", apiprocedure.GetUserName().ToString() }, "dataset");
                    break;
                default:
            ...
            if (ds3.Tables[0].Rows.Count > 0 || ds3 != null)
            {
                if (CompaireHashCode(ds3.Tables[0].Rows[0]["Password"].ToString(), txtOldPassword.Text))
                {
                    ...
                    if (ds3.Tables[0].Rows[0]["Email"].ToString() != "")
                    {
                    ...
                    else
                    {
                        ...
                        Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "alert('Email not found kindly register first!');", true);
                    }
                }
                else
                {
